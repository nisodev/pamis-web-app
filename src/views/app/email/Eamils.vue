<template>
  ...
  >
    <li
      class="flex px-7 space-x-6 group md:py-6 py-3 relative cursor-pointer hover:bg-slate-100 group items-center"
    ...
    >
      ...
      <div class="flex items-center space-x-3" v-if="item.image">
        <div class="flex-none">
      ...
      <span>
        <div class="flex-1 flex space-x-4 items-center">
          <span
            class="flex-none space-x-2 text-xs text-slate-900 dark:text-slate-300"
          >
            <span>{{ item.lastime }}</span>
          </span>
        </div>
        <span
          class="absolute right-0 top-1/2 -translate-y-1/2 dark:text-slate-300 group-hover:bg-slate-100 dark:group-hover:bg-slate-800 bg-white h-full w-[100px] flex flex-col items-center justify-center opacity-0 invisible group-hover:opacity-100 group-hover:visible"
        >
    ...
    <li
      class="mx-6"
      v-if="
        (emails.length === 0 && this.$store.state.email.search !== '') ||
        emails.length === 0
      "
    >
      <Blank :emails="emails" />
...
export default {
  ...
  methods: {
    deleteEmail(item) {
      this.$store.dispatch("deleteEmail", item);
    },
    openSingle(item) {
      this.$store.dispatch("openSingle", item);
    },
    isChecked(item) {
