<template>
  <div class="flex md:space-x-5 app_height overflow-hidden relative">
    <div
        ...
        <div class="h-full all-todos overflow-x-hidden" data-simplebar>
          ...
          >
            <div class="flex items-center space-x-3">
              ...
              <div class="max-w-[180px] flex items-center space-x-1">
              ...
              <Dropdown classMenuItems="w-[130px]">
                ...
                <template v-slot:menus>
                  ...
                  ${'hover:bg-slate-900 dark:hover:bg-slate-600 dark:hover:bg-opacity-70  dark:text-slate-300 hover:text-white'}
                   w-full border-b border-b-gray-500 border-opacity-10   px-4 py-2 text-sm  last:mb-0 cursor-pointer first:rounded-t last:rounded-b flex  space-x-2 items-center  text-slate-600 capitalize `"
                    >
...
const actions = [
  ...
  },
  {
    ...
    doit: () => {
      store.dispatch("sortZA");
    },
  },
];

// store computed
const todoSearch = computed(() => store.state.apptodo.todoSearch);

const mobileTodoSidebar = computed(() => store.state.apptodo.mobileTodoSidebar);
// dispatch openMobileTodoSidebar
const openMobileTodoSidebar = () => {
  store.dispatch("openMobileTodoSidebar");
};
const closeMobileTodoSidebar = () => {
  store.dispatch("closeMobileTodoSidebar");
};

const todos = computed(() => store.getters.todos);
const done = computed(() => store.getters.isDone);
const fav = computed(() => store.getters.isFav);
const high = computed(() => store.getters.high);
const low = computed(() => store.getters.low);
const team = computed(() => store.getters.team);
const medium = computed(() => store.getters.medium);
const update = computed(() => store.getters.update);
const trash = computed(() => store.getters.trashTodo);
  ...
  () => {
    switch (fillter.value) {
      case "all":
        ...
        store.state.apptodo.mobileTodoSidebar = false;
        setTimeout(() => {
